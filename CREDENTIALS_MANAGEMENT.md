...
`get_config` has no native vault tier — it only knows file → `.env` →
process env. The integration point is **tier 3**: fetch secrets from the
...
def load_from_vault(vault_client, mount: str, path: str) -> None:
    ...
    for key, value in secret["data"]["data"].items():
        os.environ[key.upper()] = str(value)  # get_config tries UPPER_CASE first
